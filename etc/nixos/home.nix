...
in
{
  ...
  programs.zsh = {
    ...
    in {
      ...
      ghwatch = "dotfiles_require; bash \"$DOTFILES_DIR/scripts/ghwatch.sh\"";
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      bind-key -r o run-shell "tmux display-popup -E -w 90% -h 85% sh -lc 'if [ -x \"$DOTFILES_DIR/scripts/node_modules/.bin/tsx\" ]; then \"$DOTFILES_DIR/scripts/node_modules/.bin/tsx\" \"$DOTFILES_DIR/scripts/wo/bin/tmux-wo-sessionizer.ts\"; else npx --yes tsx \"$DOTFILES_DIR/scripts/wo/bin/tmux-wo-sessionizer.ts\"; fi'"
